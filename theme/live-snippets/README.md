# Live PHP snippets (Code Snippets plugin)

| id | name | scope | state |
|---|---|---|---|
| 1 | Make upload filenames lowercase | global | inactive |
| 2 | Disable admin bar | front-end | inactive |
| 3 | Allow smilies | global | inactive |
| 4 | Current year | content | inactive |
| 5 | kaisercontainers – purge theme demo strings (site-wide) | front-end | active |
| 6 | kaisercontainers – Product itemCondition (Neu/Gebraucht) | front-end | active |
| 7 | kaisercontainers – one-time LiteSpeed purge | single-use | inactive |
| 8 | kaisercontainers – Preishinweis: zzgl. Versandkosten | front-end | active |
| 9 | kaisercontainers – Complianz configuration (one-time) | single-use | inactive |
| 10 | kaisercontainers – viewport meta (mobile rendering) | front-end | active |
| 11 | kaisercontainers – replace theme logo with brand logo | front-end | active |
| 12 | kaisercontainers – homepage meta description (DE) | front-end | active |
