# Shipping Policy — BeepWear

> Original draft. Replace every `[confirm: …]` with BeepWear's real operational values
> before publishing. Claims must match actual fulfilment. Footer-linked; summarized on PDPs.

**URL:** `/shipping-policy/`

## Order processing

Orders are prepared with care and typically dispatched within **1–2 business
days** of payment confirmation. Orders placed on weekends or public holidays are processed
the next business day. You'll receive an email when your order ships.

## Shipping methods & delivery estimates

We ship using tracked, insured carriers.

| Destination | Rate | Estimated delivery |
|-------------|------|--------------------|
| United States | **$120 flat rate**, tracked & insured | 3–7 business days |
| International | Calculated at checkout | Varies by destination |

Delivery estimates are typical transit times after dispatch, not guarantees, and may vary by
destination and customs processing.

## Tracking

Every shipment includes tracking. You'll receive a tracking link by email once your order
leaves our facility, and you can also find it in your account under **Orders**.

## International shipping & customs

We ship to: the United States and select international destinations. For international orders, **import duties, taxes, or customs fees** may apply
on delivery and are the responsibility of the recipient unless stated otherwise at checkout.

## Restrictions

We're unable to ship to certain restricted regions. Some
destinations may have carrier or regulatory restrictions we'll flag at checkout.

## Address changes

If you need to change your delivery address, contact us as soon as possible after ordering —
we can usually update it before dispatch, but not after the order has shipped.

## Delays, lost & damaged deliveries

- **Delays:** carrier or customs delays occasionally happen; we'll help you track and resolve.
- **Lost packages:** if tracking stalls, contact us and we'll open an inquiry with the carrier.
- **Damaged in transit:** photograph the packaging and item and contact us within
  **48 hours** of delivery so we can make it right.

## Questions

Contact our support team at **info@beepwear.com** — see the [Contact](/contact/) page for hours
and response times. Related: [Returns & Refunds](/returns/) · [Warranty](/warranty-policy/).
