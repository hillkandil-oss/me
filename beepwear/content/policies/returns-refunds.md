# Returns & Refunds Policy — BeepWear

> Original draft. Replace `[confirm: …]` with BeepWear's real terms before publishing.
> **URL:** `/returns/`

We want you to be confident in your purchase. If a watch isn't right, here's how returns and
refunds work.

## Return window

You may request a return within **[confirm: 14/30] days** of delivery.

## Eligibility & condition

To be eligible, the item must be:
- Unworn and undamaged, in fully resalable condition.
- Returned with all original packaging, tags, protective materials, and included items
  (box, papers, extra strap, etc.).

Items returned worn, altered, sized, or missing components may be refused or subject to a
deduction.

## Non-returnable items

[confirm: e.g. personalised/engraved items, or none.] If there are exceptions, list them
here clearly.

## How to return

1. Contact **info@beepwear.com** with your order number to request a return.
2. We'll confirm eligibility and provide return instructions [confirm: and a label, if offered].
3. Pack the watch securely in its original packaging and ship it with a tracked service.

[confirm: who pays return shipping — customer, or BeepWear for faulty/incorrect items.]

## Refund timeline

Once we receive and inspect your return, we'll notify you and process the refund to your
original payment method within **[confirm: 5–10] business days**. Your bank may take
additional time to post it.

## Exchanges

Prefer a different reference, size, or strap? Contact us — we'll arrange an exchange subject
to availability, following the return process above.

## Damaged, faulty, or incorrect orders

If your order arrives damaged, faulty, or incorrect, contact us within **[confirm: 48 hours]**
with photos. We'll cover return shipping and send a replacement or full refund — your choice.

## Contact

**info@beepwear.com** · [Contact](/contact/). Related: [Shipping](/shipping-policy/) ·
[Warranty](/warranty-policy/).
